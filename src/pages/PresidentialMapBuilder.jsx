...
export default function PresidentialMapBuilder() {
  ...
  const [activeParty, setActiveParty] = useState(null); // 'D' | 'R' | null
  ...
  const handleStateClick = (abbr) => {
    if (activeParty === 'D') {
      ...
      const next = idx === -1 ? dCycle[0] : dCycle[(idx + 1) % dCycle.length];
      setRatings(r => ({ ...r, [abbr]: next }));
    } else if (activeParty === 'R') {
      const cur = getRating(abbr);
      const idx = rCycle.indexOf(cur);
      const next = idx === -1 ? rCycle[0] : rCycle[(idx + 1) % rCycle.length];
      setRatings(r => ({ ...r, [abbr]: next }));
    }
  };
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mb-6">
          ...
          {!activeParty && (
            <p className="text-white/40 text-xs text-center mt-2">Select a party button above, then click states</p>
          )}
        </div>
